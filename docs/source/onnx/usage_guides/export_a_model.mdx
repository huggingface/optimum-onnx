...
QuestionAnsweringModelOutput(
    loss=None,
    start_logits=tensor(
       [[-4.7652, -1.0452, -7.0409, -4.6864, -4.0277, -6.2021, -4.9473,  2.6287,
          7.6111, -1.2488, -2.0551, -0.9350,  4.9758, -0.7707,  2.1493, -2.0703,
         -4.3232, -4.9472]]),
    end_logits=tensor(
       [[ 0.4382, -1.6502, -6.3654, -6.0661, -4.1482, -3.5779, -0.0774, -3.6168,
...
## Opset and dynamo exporter

Two onnx exporters coexists in [torch.onnx.export](https://docs.pytorch.org/docs/stable/onnx_export.html#torch.onnx.export).
The first version enabled with `dynamo=False` is recommended for any ONNX opset < 18.
This exporter is based on [TorchScript](https://docs.pytorch.org/docs/stable/jit.html) which is deprecated.

The new version enabled with `dynamo=True` (or `--dynamo` with `optimum-cli`) is recommended
...
Moreover, the advanced argument `fn_get_submodels` to
[main_export](https://huggingface.co/docs/optimum-onnx/onnx/package_reference/export#optimum.exporters.onnx.main_export)
allows to customize how the submodels are extracted in case the model needs to be exported in several submodels.
