...
The `optimum.onnxruntime.ORTModelForXXX` model classes are API compatible with Hugging Face Transformers models.
This means you can just replace your `AutoModelForXXX` class with the corresponding `ORTModelForXXX` class in `optimum.onnxruntime`.
...
https://huggingface.co/docs/optimum/main/en/onnxruntime/package_reference/modeling_ort
...
## Converting your model on-the-fly
...
base = ORTDiffusionPipeline.from_pretrained(model_id)
prompt = "sailing ship in storm by Leonardo da Vinci"
...
model_id = "stabilityai/stable-diffusion-xl-refiner-1.0"
refiner = ORTDiffusionPipeline.from_pretrained(model_id, export=True)
