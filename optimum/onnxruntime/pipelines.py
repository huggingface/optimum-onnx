...
def ort_infer_framework_load_model(
    ...
    **model_kwargs,
    ...
    model_kwargs.pop("torch_dtype", None)  # we don't support model export to different dtypes yet
    model_kwargs.pop("_commit_hash", None)  # not needed for ORTModel

    ort_model_class = ORT_SUPPORTED_TASKS[task]["class"][0]

    if model is None:
        model_id = ORT_SUPPORTED_TASKS[task]["default"]
        ort_model = ort_model_class.from_pretrained(model_id, **model_kwargs)
...
# The doc is simply a copy of transformers.pipelines.pipeline's doc with minor modifications
