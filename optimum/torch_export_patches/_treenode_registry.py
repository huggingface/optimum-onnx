...
def register_class_serialization(
    ...
    if pv.Version(torch.__version__) < pv.Version("2.7"):
        if verbose:
            print(f"[register_class_serialization] ---------- register {cls.__name__} for torch=={torch.__version__}")
...
def register_cache_serialization(
    ...
    for cls, version in wrong.items():
        if (
            ...
            and (version is None or pv.Version(transformers.__version__) >= pv.Version(version))
        ):
...
def serialization_functions(
    ...
    for cls in supported_classes:
        ...
        classes[cls] = lambda verbose=verbose, _ln=lname, cls=cls, _al=all_functions: register_class_serialization(
            cls,
            _al[f"flatten_{_ln}"],
...
def unregister_cache_serialization(undo: dict[str, bool], verbose: int = 0):
    ...
    cls_ensemble = {MambaCache, cache_utils.DynamicCache, cache_utils.EncoderDecoderCache} | set(undo)
