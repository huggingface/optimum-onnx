...
def make_hybrid_cache(
    ...
    if key_value_pairs:
        ...
        if len(sets_of_dim) == 1:
            ...
            assert len(sets_of_dim) == 2, f"Not implemented for more than 2 dimensions {sets_of_dim}"
    ...
    for i, (key, value) in enumerate(key_value_pairs):
        cache.update(
            ...
            value,
            i,
            cache_kwargs={"cache_position": torch.arange(0, key.shape[2], dtype=torch.int64).to(key.device)},
        )
    if hasattr(cache, "layers") and len(key_value_pairs) < len(cache.layers):
        # The cache constructor contains the two following lines
        # (in cache_utils.py) which append empty layers when the cache is
...
def flatten_unflatten_for_dynamic_shapes(
    ...
    for subspec in spec.children_specs:
        ...
        value = flatten_unflatten_for_dynamic_shapes(value, use_dict=use_dict, change_function=change_function)
