...
from optimum.torch_export_patches import _cache_creator
...
def flatten_mamba_cache(
    ...
    assert isinstance(mamba_cache.conv_states, list) and isinstance(mamba_cache.ssm_states, list), (
...
def unflatten_static_cache(values: list[Any], context: torch.utils._pytree.Context, output_type=None) -> StaticCache:
    ...
    return _cache_creator.make_static_cache(list(zip(values[0], values[1])), max_cache_len=values[0][0].shape[2])
...
def make_serialization_function_for_dataclass(
    ...
    def unflatten_cls(values: list[Any], context: torch.utils._pytree.Context, output_type=None) -> cls:  # type: ignore[valid-type]
